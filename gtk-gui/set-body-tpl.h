 ...
 * $HeadURL: svn://lunix120.ensiie.fr/yfig/trunk/include/tools/set-body-template.h $
...
{
    TPLSET_SetType* ret = gui_malloc(sizeof(*ret));
    ret->eleMemNb  = n>=0 ? n : 0; 
    ret->eleNb     = 0;
    ret->eles      = n>0  ? gui_malloc(sizeof(*ret->eles)*n) : 0;
...
extern void            TPLSET_Func(appendT)(TPLSET_SetType* d, TPLSET_EleType* x, int n)
{
    ...
    if ( d->eleNb > d->eleMemNb ) {
        d->eleMemNb = d->eleNb;
        d->eles = gui_realloc(d->eles,sz*d->eleMemNb);
    }
...
#endif
{
    if ( s->eleNb == s->eleMemNb ) {
        s->eleMemNb += 4;
        s->eles = gui_realloc(s->eles,sizeof(*s->eles)*s->eleMemNb);
    }
#ifndef TPLSET_AddWithoutPtr
    s->eles[s->eleNb] = *x;
#else
...
{
    int i;
    TEMP_asf_printf(pos<0 || s->eleNb<=pos, "pos out of range [0:%d[\n",s->eleNb);
    if ( s->eleNb == s->eleMemNb ) {
        s->eleMemNb += 4;
        s->eles = gui_realloc(s->eles,sizeof(*s->eles)*s->eleMemNb);
    }
...
extern void            TPLSET_Func(movI2Pshift) (TPLSET_SetType* s, int idx, int pos)
{
    ...
    TEMP_asf_printf(idx<0 || s->eleNb<=idx, "idx out of range [0:%d[\n",s->eleNb);
    TEMP_asf_printf(pos<0 || s->eleNb<=pos, "pos out of range [0:%d[\n",s->eleNb);
    if ( idx<pos ) {
        //   ----I-------P--------
        tmp = s->eles[idx];
